...
######## Sampling 100 times ########
n_samples = 100
posterior_sentences = sample_perturbation_posterior_sentences( 
...
expr_samples = posterior_sentences_to_expression( 
    sentences=posterior_sentences, 
    vocab_list=vocab_list, 
    slope=slope,
    intercept=intercept, 
)
print(expr_samples.shape)
# expr_samples: (n_samples, n_genes)

posterior_mean = expr_samples.mean(axis=0)
posterior_low  = np.quantile(expr_samples, 0.025, axis=0)
posterior_high = np.quantile(expr_samples, 0.975, axis=0)
...
######## Save the results ########
with open(os.path.join(benchmarking_output_path, "posterior_samples_meta.pkl"), "wb") as f:
    pickle.dump(
        {
            "posterior_samples": posterior_sentences,  # list[str]
            "expr_samples": expr_samples,              # np.ndarray
            "prompt": inference_prompt,
            "n_samples": len(posterior_sentences),
        },
        f,
    )
